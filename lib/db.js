...
// Sync/Seed existing markdown files into MongoDB ONLY on initial setup if database is completely empty
export async function syncLocalPostsToMongo(postsDir) {
  try {
    ...
    // Only seed from disk if database is brand new and has 0 posts
    if (count > 0) {
      return;
    }
    ...
    console.log(`Initial seed of ${files.length} local posts to empty MongoDB Atlas collection.`);
  } catch (err) {
